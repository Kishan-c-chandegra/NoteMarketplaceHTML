...
namespace NotesMarketPlace.Controllers
{
    ...
    public class SearchNotesController : Controller
    {
        ...
        public ActionResult Notes(int id)
        {
            ...
            // get seller
            var seller = _dbcontext.Users.Where(x => x.UserID == NoteDetail.SellerID).FirstOrDefault();
            ...
            IEnumerable<ReviewsViewModel> reviews = from review in _dbcontext.SellerNotesReviews
                                                    join users in _dbcontext.Users on review.ReviewedByID equals users.UserID
            ...
            if (user != null)
            {
                notesdetail.UserID = user.UserID;
            }
            ...
            if (User.Identity.IsAuthenticated)
            {
                ...
                var request = _dbcontext.Downloads.Where(x => x.NoteID == id && x.Downloader == user.UserID && x.IsSellerHasAllowedDownload == false && x.AttachementPath == null).FirstOrDefault();
                // if logged in user is allow download this note
                var allowdownloadnotes = _dbcontext.Downloads.Where(x => x.NoteID == id && x.Downloader == user.UserID && x.IsSellerHasAllowedDownload == true && x.AttachementPath != null).FirstOrDefault();
        ...
        public void RequestPaidNotesTemplate(Download download, User user)
        {
            ...
            //get seller
            var seller = _dbcontext.Users.Where(x => x.UserID == download.Seller).FirstOrDefault();
