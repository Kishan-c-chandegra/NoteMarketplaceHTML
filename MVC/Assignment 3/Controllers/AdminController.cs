...
namespace NotesMarketPlace.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        public ActionResult Dashboard(string search, string sort, string month, int page = 1)
        {
            ...
            // get memberid
            int memberid = _dbcontext.UserRoles.Where(x => x.Name.ToLower() == "member").Select(x => x.ID).FirstOrDefault();
            ...
            var notedownloaded = _dbcontext.Downloads.Where(x => x.AttaachementDownloadID > last7days).Count();
            ...
            foreach (var item in publishednotelist)
            {
                ...
                var attachement = _dbcontext.SellerNotesAttachments.Where(x => x.NoteID == item.ID);
                ...
                // get seller
                var publisher = _dbcontext.Users.Where(x => x.UserID == item.SellerID).First();
