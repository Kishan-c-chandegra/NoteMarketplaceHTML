...
namespace NotesMarketPlace.Controllers
{
    ...
    public class SellYourNotesController : Controller
    {
        ...
        public ActionResult Dashboard(string search1, string search2, string sort1, string sort2, int page1 = 1, int page2 = 1)
        {
            ...
            dashboardviewmodel.NumberOfSoldNotes = _dbcontext.Downloads.Where(x => x.Seller == user.UserID && x.IsSellerHasAllowedDownload == true && x.AttachementPath != null).Count();
            dashboardviewmodel.MoneyEarned = _dbcontext.Downloads.Where(x => x.Seller == user.UserID && x.IsSellerHasAllowedDownload == true && x.AttachementPath != null).Select(x => x.PurchasedPrice).Sum();
            dashboardviewmodel.MyDownloads = _dbcontext.Downloads.Where(x => x.Downloader == user.UserID && x.IsSellerHasAllowedDownload == true && x.AttachementPath != null).Count();
            dashboardviewmodel.MyRejectedNotes = _dbcontext.SellerNotes.Where(x => x.SellerID == user.UserID && x.Status == rejectedid && x.IsActive == true).Count();
            dashboardviewmodel.BuyerRequest = _dbcontext.Downloads.Where(x => x.Seller == user.UserID && x.IsSellerHasAllowedDownload == false && x.AttachementPath == null).Count();

            // get inprogress notes
            dashboardviewmodel.InProgressNotes = from note in _dbcontext.SellerNotes
                                                 where (note.Status == draftid || note.Status == submittedforreviewid || note.Status == inreviewid) && note.SellerID == user.UserID
            ...
            dashboardviewmodel.PublishedNotes = from note in _dbcontext.SellerNotes
                                                where note.Status == publishedid && note.SellerID == user.UserID
        ...
        public ActionResult PublishNote(int id)
        {
            ...
            if (user.UserID == note.SellerID)
            {
                ...
                PublishNot(note.Title, sellername);
            }
