    ...
    public class AdminNotesController : Controller
    {
        ...
        public ActionResult NotesUnderReview(int? member, int? seller, string search, string sort, int page = 1)
        {
            ...
            if (member != null)
            {
                ...
                notelist = from notes in _dbcontext.SellerNotes
                           join sellers in _dbcontext.Users on notes.SellerID equals sellers.UserID
                           where (notes.Status == submittedforreviewid || notes.Status == inreviewid) && sellers.UserID == member
        ...
        public ActionResult PublishedNotes(int? member, int? seller, string search, string sort, int page = 1)
        {
            ...
            if (member != null)
            {
                ...
                notelist = from notes in _dbcontext.SellerNotes
                           join sellers in _dbcontext.Users on notes.SellerID equals sellers.UserID
                           join publishedby in _dbcontext.Users on notes.ActionedBy equals publishedby.UserID
                           ...
                           select new PublishedNoteViewModel
                           {
                               ...
                               NumberOfDownloads = downloadlist.Where(x => x.IsSellerHasAllowedDownload == true && x.AttachementPath != null).Count()
                           };
            }
        ...
        public ActionResult DownloadedNotes(int? selectednote, int? member, int? note, int? seller, int? buyer, string search, string sort, int page = 1)
        {
            ...
            if (selectednote != null)
            {
                ...
                notelist = from downloads in _dbcontext.Downloads
                           join sellers in _dbcontext.Users on downloads.Seller equals sellers.UserID
                           join buyers in _dbcontext.Users on downloads.Downloader equals buyers.UserID
                           where downloads.IsSellerHasAllowedDownload == true && downloads.AttachementPath != null && downloads.NoteID == selectednote
            ...
            else if (member != null)
            {
                ...
                notelist = from downloads in _dbcontext.Downloads
                           join sellers in _dbcontext.Users on downloads.Seller equals sellers.UserID
                           join buyers in _dbcontext.Users on downloads.Downloader equals buyers.UserID
                           where downloads.IsSellerHasAllowedDownload == true && downloads.AttachementPath!= null && downloads.Downloader == member
                           select new DownloadedNotesViewModel
                           {
                               NoteID = downloads.NoteID,
                               SellerID = sellers.UserID,
            ...
            else
            {
                notelist = from downloads in _dbcontext.Downloads
                           join sellers in _dbcontext.Users on downloads.Seller equals sellers.UserID
                           join buyers in _dbcontext.Users on downloads.Downloader equals buyers.UserID
                           where downloads.IsSellerHasAllowedDownload == true && downloads.AttachementPath != null
                           select new DownloadedNotesViewModel
                           {
                               NoteID = downloads.NoteID,
                               SellerID = sellers.UserID,
        ...
        public ActionResult RejectedNotes(int? seller, string search, string sort, int page = 1)
        {
            ...
            IEnumerable<RejectedNotesViewModel> notelist = from notes in _dbcontext.SellerNotes
                                                           join sellers in _dbcontext.Users on notes.SellerID equals sellers.UserID
                                                           join rejectedby in _dbcontext.Users on notes.ActionedBy equals rejectedby.UserID
