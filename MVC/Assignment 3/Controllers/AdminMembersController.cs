...
namespace NotesMarketPlace.Controllers
{
    ...
    public class AdminMembersController : Controller
    {
        ...
        public ActionResult Members(string search, string sort, int page = 1)
        {
            ...
            var userrolememberid = _dbcontext.UserRoles.Where(x => x.Name.ToLower() == "member").Select(x => x.ID).FirstOrDefault();
            ...
            IEnumerable<MembersViewModel> memberlist = from member in _dbcontext.Users
                                                       ...
                                                       select new MembersViewModel
                                                       {
                                                           ...
                                                           UnderReviewNotes = _dbcontext.SellerNotes.Where(x => x.SellerID == member.UserID && (x.Status == submittedforreviewid || x.Status == inreviewid)).Count(),
                                                           PublishedNotes = _dbcontext.SellerNotes.Where(x => x.SellerID == member.UserID && x.Status == publishednoteid).Count(),
                                                           DownloadedNotes = _dbcontext.Downloads.Where(x => x.Downloader == member.UserID && x.IsSellerHasAllowedDownload == true).Count(),
                                                           TotalExpenses = _dbcontext.Downloads.Where(x => x.Downloader == member.UserID && x.IsSellerHasAllowedDownload == true).Select(x => x.PurchasedPrice).Sum(),
                                                           TotalEarning = _dbcontext.Downloads.Where(x => x.Seller == member.UserID && x.IsSellerHasAllowedDownload == true).Select(x => x.PurchasedPrice).Sum()
                                                       };
        ...
        public ActionResult MemberDetail(int member, string sort, int page = 1)
        {
            ...
            // get member
            User users = _dbcontext.Users.Where(x => x.UserID == member).FirstOrDefault();
            // get member's profile
            UserProfile userprofiles = _dbcontext.UserProfiles.Where(x => x.UserID == users.UserID).FirstOrDefault();
            // get member's notes excluding note status draft
            var notes = _dbcontext.SellerNotes.Where(x => x.SellerID == users.UserID && x.Status != draftid).ToList();
            ...
            foreach (var note in notes)
            {
                ...
                var downloadednoteslist = _dbcontext.Downloads.Where(x => x.NoteID == note.ID && x.IsSellerHasAllowedDownload == true && x.AttachementPath != null);
